...
export const createRestaurant = (reqData) => {
    return async (dispatch) => {
        ...
        // validation
        if (!reqData.token || reqData.jwt) {
            console.error("No token provided");
            dispatch({
                type: CREATE_RESTAURANT_FAILURE,
                payload: new Error("No authentication token provided")
            });
            return;
        }
        
        try {
            console.log("Sending request with token:", reqData.token);
            const { data } = await api.post(`/api/admin/restaurants`,reqData.data || reqData.jwt, {
                headers: {
                    Authorization: `Bearer ${reqData.token || reqData.jwt}`,
                },
            });
...
export const updateRestaurantStatus = ({ restaurantId, jwt }) => {
    return async (dispatch) => {
        ...
        try {
            ...
            console.log("error ", error);
            dispatch({typea: UPDATE_RESTAURANT_STATUS_FAILURE, payload:error});
        }
    };
};
    ...
    return async (dispatch) => {
        ...
        try {
            const res = await api.post(
                `/api/admin/events/resstaurants/${restaurantId}`,
                data,
                {
                    headers: {
                        Authorization: `Bearer ${jwt}`,
